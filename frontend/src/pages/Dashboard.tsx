...
export const Dashboard = () => {
    ...
    const fetchPermissions = async () => {
        ...
        setRefreshing(true);
        try {
            // 1. Fetch actual permissions/profile
            const meResponse = await apiClient.get('/auth/me', {
                params: { company_id: destCompanyId }
            });
            if (meResponse.data.allowed_apps) {
                setPermissions(meResponse.data.allowed_apps);
                setIsAdmin(meResponse.data.is_admin);
            }

            // 2. Fetch pending requests to show "Requested" status
            const reqResponse = await apiClient.get('/admin/my-requests', {
                params: { company_id: destCompanyId }
            });
            const pendingSlugs = reqResponse.data
                .filter((r: any) => r.status === 'pending')
                .map((r: any) => r.app_slug);
            setRequestedApps(new Set(pendingSlugs));
        } catch (error) {
    ...
    const handleRequestAccess = async (appKey: string, appName: string) => {
        ...
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                setRequestedApps(prev => new Set(prev).add(appKey));
            }
        } catch (error: any) {
            console.error('Request access error:', error);
            const detail = error.response?.data?.detail || 'Failed to send access request.';
    ...
    const apps = [
        ...
        },
        {
            ...
            component: <UniversalMigrationPage 
        ...
        },
        {
            ...
            component: <UniversalMigrationPage 
                ...
                supportedLevels={['project']} // Daily log is only project level
            />,
        },
        {
            key: 'drawings',
                                    ...
                                    <div className="flex items-center gap-2">
                                        {requestedApps.has(app.key) ? (
                                            ...
                                            <Button 
                                                ...
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    handleRequestAccess(app.key, app.name);
                                                }}
