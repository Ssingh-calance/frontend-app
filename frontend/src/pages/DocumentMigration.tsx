...
type MappingRecord = Record<string, MappingInfo>;
...
const FolderTreeItem = ({
    ...
    const folderSyncInfo = mappings[String(folder.id)];
    const isFolderSynced = folderSyncInfo?.status === 'success';

    const initialSubfolders = useMemo(() => folder.folders ?? [], [folder.folders]);
    const initialFiles = useMemo(() => folder.files ?? [], [folder.files]);

    const subfoldersFromList = useMemo(
        () =>
            allFolders.filter(
                (f) => f.parent_id === folder.id && !initialSubfolders.some((s) => s.id === f.id)
            ),
        [allFolders, folder.id, initialSubfolders]
    );

    const combinedSubfolders = useMemo(
        () => [...initialSubfolders, ...subfoldersFromList],
        [initialSubfolders, subfoldersFromList]
    );

    const shouldLoadFiles = isExpanded && !!contextId && initialFiles.length === 0;

    const { data: lazyFiles, isLoading: loadingFiles } = useQuery<FileData[]>({
    ...
    const displayFiles: FileData[] = initialFiles.length > 0 ? initialFiles : (lazyFiles ?? []);
        ...
        <div className="mb-1">
            ...
            {isExpanded && (
                <div className="ml-6 border-l border-zinc-700 pl-4 mt-1 space-y-1">
                    {combinedSubfolders.map((subfolder) => (
                        <FolderTreeItem
                            key={subfolder.id}
                            scope={scope}
                            contextId={contextId}
                    ...
                    {displayFiles.map((file) => {
                        const fileSyncInfo = mappings[String(file.id)];
...
const MigrationTab = ({
    ...
    const { data: folders, isLoading: loadingFolders } = useQuery<FolderData[]>({
    ...
    const { data: mappings = {}, refetch: refetchMappings } = useQuery<MappingRecord>({
        ...
        queryFn: async () => {
            const params: { dest_company_id: number; dest_project_id?: number } = { dest_company_id: destId };
            if (scope === 'project') {
                params.dest_project_id = destId;
            }
            const res = await apiClient.get('/documents/mappings', { params });
            return res.data as MappingRecord;
        },
        enabled: !!destId
    });
    ...
    const syncMutation = useMutation({
        ...
        onError: (err: unknown) => {
            message.error(getErrorMessage(err, 'Failed to sync folder'));
        }
    });
    ...
    const syncFileMutation = useMutation({
        ...
        onError: (err: unknown) => {
            message.error(`File migration failed: ${getErrorMessage(err, 'Unknown error')}`);
        }
    });
    ...
    const rootFolder = useMemo(() => {
        ...
        return folders?.find(
            (f) =>
                !f.parent_id &&
                (f.name.toLowerCase() === sourceName.toLowerCase() || f.name.includes(sourceName))
        );
    ...
    const displayFolders = useMemo(() => {
        if (!folders) return [];
        if (rootFolder) {
            const rootChildren = rootFolder.folders ?? [];
    ...
    const displayFiles = useMemo<FileData[]>(() => rootFolder?.files ?? [], [rootFolder]);
                ...
                title={(
                    <div className="flex justify-between items-center w-full py-1">
                        <Space>
                            ...
                            <span className="text-zinc-200">Folders in {sourceName}</span>
                        </Space>
                        <Button
                            type="primary"
                    ...
                    <div className="max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        {displayFiles.map((file) => (
                            <div key={file.id} className="flex items-center justify-between group ml-10 py-1 hover:bg-zinc-800/50 rounded px-2">
                                ...
                                <Button type="link" size="small" onClick={() => syncFileMutation.mutate(file.id)} className="opacity-0 group-hover:opacity-100">
                                    Sync
                                </Button>
                            </div>
                        ))}
...
export const DocumentMigration = () => {
    ...
    const [destProject, setDestProject] = useState<number | null>(null);
